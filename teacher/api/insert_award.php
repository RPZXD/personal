...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($tid) && !empty($award) && !empty($date1) && !empty($term) && !empty($year)) {
        try {
            $result = $person->insertAwardDetails($tid, $award, $level, $date1, $term, $year, $department, $certificate);
            if ($result) {
                $response['success'] = true;
                $response['message'] = 'เพิ่มรายละเอียดรางวัลเรียบร้อยแล้ว';
            } else {
                $response['message'] = 'ไม่สามารถเพิ่มรายละเอียดรางวัลได้';
            }
        } catch (Exception $e) {
